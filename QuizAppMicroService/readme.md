this repository contains the microservice structure to the quiz app

1, QuestionService - runs in port 8080 and has all endpoinds to get, generate, calculate score for questions, to load initial data run the below in SQL server after running QuestionService
...
2. QuizService - runs in port 8090, it calls questions service using OpenFeign, and is responsible for get, create and submit quiz

3. service-registry - runs in port 8761, it uses eureka server. here all the projects with eureka servers clients are registered

4. gateway - runs in port 8765, it is responsible for providing a single port to all the services registerd to eureka client in service-registry. currently its not working
